function a_H= HMM(a, iota, delta, center, p, meshes, free, phix, phixx, T)
...
for i = 1: size(meshes, 2)
	...
	jacobi = T(v);
	for j = 1: size(wg, 2)
		A(meshes(:, i), meshes(:, i)) = A(meshes(:, i), meshes(:, i)) + wg(j) * jacobi * (iota^2 * phixx(ld(:, j), v) * phixx(ld(:, j), v)' + phix(ld(:, j), v) * a(ld(:, j), v) * phix(ld(:, j), v)'); 
	end
end
A = sparse(A); %Nv-by-Nv
