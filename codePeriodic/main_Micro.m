...
for gamma = [1/4, 1, 4] %[1/4, 1/2, 3/4, 1, 3/2, 2, 5/2, 3] 
	fprintf("%.2f", gamma);
...
%{
		...
		a_bar = [3/2, 0; 0, 5/3];
%}
		[p_h, meshes_h, trans_h] = init_mesh(h); %2-by-Nv; 1-by-Nv, 9-by-Nt
		parfor i = 1: size(vertices_H, 2)
			a_H(:, :,  i) = HMM(a, iota, delta, vertices_H(:, i), p_h, meshes_h, trans_h, phix, phixx, T);
		end
